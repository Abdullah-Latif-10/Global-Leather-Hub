      ...
      <div className="min-h-screen bg-canvas pt-24 pb-16">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="card-linen flex flex-col items-center justify-center py-16 text-center border-rust/20">
            ...
            <p className="text-espresso font-medium">{error || "Product not found"}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-canvas text-espresso pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        {/* Top Navigation */}
        <Link to="/products" className="btn-ghost mb-8 inline-flex animate-fade-in group text-xs text-fog uppercase tracking-widest hover:text-espresso">
          <ArrowLeft className="w-4 h-4 mr-2 transition-transform group-hover:-translate-x-1" /> Back to Catalog
        </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
                ...
                {product.images.map((img, idx) => (
                  <button
                    ...
                    className={`aspect-square rounded-xl overflow-hidden border-2 transition-all duration-200 ${
                      activeImage === idx ? "border-tan ring-2 ring-tan/20 shadow-md transform scale-[1.02]" : "border-transparent opacity-70 hover:opacity-100"
                    }`}
                  >
          ...
          <div className="animate-fade-left lg:py-4 flex flex-col">
            <h1 className="leading-tight text-3xl md:text-4xl mb-4 text-espresso" style={{ fontFamily: '"Playfair Display", serif', fontWeight: 500 }}>
              {product.name}
            </h1>
            ...
            <div className="bg-linen/40 border border-border rounded-2xl p-6 mb-8 shadow-sm">
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  <span className="block text-[10px] text-fog uppercase tracking-widest font-semibold mb-1">Minimum Order Qty</span>
                  <span className="text-espresso font-medium text-xl flex items-center gap-2">
                    {product.moq} <span className="text-sm text-fog font-normal">units</span>
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-10 mb-8">
              {product.material && (
                <div className="flex gap-3">
                  ...
                  <div>
                    <h4 className="text-[11px] font-semibold uppercase tracking-widest text-fog mb-1">Material</h4>
                    <p className="text-sm text-espresso font-medium">{product.material}</p>
                  </div>
                </div>
              )}
              {product.fit && (
                <div className="flex gap-3">
                  ...
                  <div>
                    <h4 className="text-[11px] font-semibold uppercase tracking-widest text-fog mb-1">Fit/Cut</h4>
                    <p className="text-sm text-espresso font-medium">{product.fit}</p>
                  </div>
                </div>
              )}
              {product.availableSizes && product.availableSizes.length > 0 && (
                <div className="flex gap-3">
                  ...
                  <div>
                    <h4 className="text-[11px] font-semibold uppercase tracking-widest text-fog mb-1">Available Sizes</h4>
                    <p className="text-sm text-espresso font-medium">{product.availableSizes.join(", ")}</p>
                  </div>
                </div>
              )}
              {product.availableColors && product.availableColors.length > 0 && (
                <div className="flex gap-3">
                  ...
                  <div>
                    <h4 className="text-[11px] font-semibold uppercase tracking-widest text-fog mb-1">Available Colors</h4>
                    <p className="text-sm text-espresso font-medium">{product.availableColors.join(", ")}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            {product.highlights && product.highlights.length > 0 && (
              <div className="mb-8">
                <h4 className="text-[11px] font-semibold uppercase tracking-widest text-fog mb-3">Key Features</h4>
                <ul className="space-y-2">
                  {product.highlights.map((item, idx) => (
                    <li key={idx} className="flex gap-3 text-sm text-espresso items-start">
            ...
            {product.specifications && product.specifications.length > 0 && (
              <div className="mb-10">
                <h4 className="text-[11px] font-semibold uppercase tracking-widest text-fog mb-3">Technical Specifications</h4>
                <div className="bg-paper border border-border rounded-xl divide-y divide-border">
                  {product.specifications.map((spec, idx) => (
                    <div key={idx} className="flex justify-between py-3 px-4 text-sm">
                      <span className="text-fog">{spec.label}</span>
                      <span className="text-espresso font-medium text-right max-w-[60%]">{spec.value}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="mt-auto space-y-4 pt-4 border-t border-border">
              ...
              <Link to="/contact" className="btn-outline w-full justify-center text-[13px] py-3.5 border-transparent bg-linen/50">
