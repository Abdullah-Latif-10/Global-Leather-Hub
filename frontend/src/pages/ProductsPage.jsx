...
export default function ProductsPage() {
  ...
  const hasActiveFilters = searchParams.get("search") || searchParams.get("category") || searchParams.get("minPrice") || searchParams.get("maxPrice");
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        <div className="mb-10 reveal visible">
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            <div>
              <h1 className="leading-tight" style={{ fontFamily: '"Playfair Display", serif', fontSize: "clamp(2rem, 4vw, 3.25rem)", fontWeight: 400 }}>
                Wholesale Product
                <em style={{ fontStyle: "italic", color: "#8B5E3C" }}> Catalogue</em>
              </h1>
              <p className="text-fog text-sm mt-4 max-w-2xl leading-relaxed">
                Explore our high-margin leather products curated for wholesalers.
                Request quotes with your MOQ and branding requirements.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          {/* Sidebar Filters */}
          <aside className={`w-full lg:w-64 flex-shrink-0 transition-all duration-300 overflow-hidden lg:overflow-visible ${showMobileFilters ? "max-h-[1000px] opacity-100 mb-6" : "max-h-0 lg:max-h-none opacity-0 lg:opacity-100 m-0"}`}>
            <div className="sticky top-28 space-y-6 bg-paper p-6 rounded-2xl border border-border shadow-soft">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium tracking-wide" style={{ fontFamily: '"Playfair Display", serif' }}>Filters</h3>
                {hasActiveFilters && (
                  <button onClick={clearFilters} className="text-[11px] text-rust hover:underline transition-all">Clear All</button>
                )}
              </div>
              ...
              <form onSubmit={handleFilter} className="space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-[11px] font-semibold tracking-widest uppercase text-fog">Search</label>
                ...
                {/* Category */}
                <div className="space-y-2">
                  <label className="text-[11px] font-semibold tracking-widest uppercase text-fog">Category</label>
                  <select
                    value={category}
                ...
                {/* Price Range */}
                <div className="space-y-2">
                  <label className="text-[11px] font-semibold tracking-widest uppercase text-fog">Price Range (USD)</label>
                ...
                {/* Sort */}
                <div className="space-y-2">
                  <label className="text-[11px] font-semibold tracking-widest uppercase text-fog">Sort By</label>
                  <select
                    value={sort}
                ...
                <button type="submit" className="btn-primary w-full py-3 justify-center text-xs mt-4">
                  Apply Filters
                </button>
              </form>
            </div>
          ...
          <main className="flex-1">
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-32 opacity-70">
                ...
                <p className="text-fog text-sm animate-pulse">Curating catalogue...</p>
              </div>
            ) : error ? (
              <div className="card-linen flex flex-col items-center justify-center py-16 text-center border-rust/20">
                ...
                <button onClick={fetchProducts} className="btn-outline mt-4 text-xs py-2">Try Again</button>
              </div>
            ) : products.length === 0 ? (
              <div className="card text-center py-20 border-dashed border-2">
                <FileX className="w-16 h-16 text-fog/30 mx-auto mb-5" />
                <h3 className="text-espresso text-xl mb-2" style={{ fontFamily: '"Playfair Display", serif' }}>No Models Found</h3>
                ...
                {products.map((item, idx) => (
                  ...
                  >
                    ...
                    <div className="p-6 flex flex-col flex-grow">
                      ...
                      <h2 className="text-espresso text-lg mb-2 leading-tight flex-grow" style={{ fontFamily: '"Playfair Display", serif', fontWeight: 500 }}>
                        {item.name}
                      </h2>
            ...
            {pagination.pages > 1 && (
              <div className="mt-12 flex items-center justify-center gap-4">
                ...
                <span className="text-sm font-medium text-fog px-4 bg-paper py-2 rounded-full border border-border shadow-soft">
                  <span className="text-espresso">{pagination.page}</span> / {pagination.pages}
                </span>
        ...
        <div className="mt-16 card-linen rounded-2xl p-6 md:p-8 animate-fade-up style-delay-300">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h3 className="text-espresso text-xl mb-1" style={{ fontFamily: '"Playfair Display", serif', fontWeight: 400 }}>
                Need Custom Branding?
              </h3>
              <p className="text-fog text-sm">
                Private label, debossing, and custom hardware options available across all lines.
              </p>
            </div>
            <div className="flex gap-3 flex-wrap items-center">
              ...
              <Link to="/contact" className="btn-espresso bg-espresso text-paper px-6 py-2.5 rounded-full text-[13px] font-medium hover:bg-sienna transition-all shadow-sm">
                Talk to Sales
              </Link>
            </div>
          </div>
