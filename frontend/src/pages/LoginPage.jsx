  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      const { data } = await api.post("/auth/login", {
        email: form.email, password: form.password, cfTurnstileToken: cfToken,
      });
      login(data.data);
      ...
      if (rd?.needsVerification) { toast.error("Email not verified."); navigate("/register"); return; }
      if (rd?.errors) {
        const fe = {};
        rd.errors.forEach((e) => { fe[e.field] = e.message; });
        setErrors(fe);
      } else toast.error(rd?.message || "Login failed. Please try again.");
    ...
    <div className="min-h-screen bg-canvas flex page-enter">
      ...
      <div className="hidden lg:block lg:w-[55%] relative overflow-hidden">
        ...
        <div className="absolute top-10 left-10 animate-fade-down stagger-2">
          <Link to="/">
            <span className="text-paper/90 text-2xl" style={{ fontFamily: '"Playfair Display", serif', fontWeight: 400 }}>
              Global Leather Hub
            </span>
            <p className="text-paper/40 text-[10px] tracking-[0.3em] uppercase mt-0.5">Wholesale Platform</p>
          </Link>
        </div>
        ...
        <div className="absolute bottom-10 left-10 right-16 animate-fade-up stagger-3">
          <h2
            ...
            style={{ fontFamily: '"Playfair Display", serif', fontSize: 'clamp(1.8rem, 3vw, 2.8rem)' }}
          >
            Premium Leather,
            <br />
            <em style={{ fontStyle: 'italic', color: '#C9A97A' }}>Global Reach.</em>
          </h2>
          <p className="text-paper/55 text-sm font-light leading-relaxed max-w-xs">
            Exclusive wholesale pricing on jackets, belts, and wallets since 2009.
          </p>
          <div className="flex gap-8 mt-6">
            {[{ v: "50+", l: "Countries" }, { v: "10K+", l: "Products" }, { v: "5K+", l: "Partners" }].map(({ v, l }) => (
              <div key={l}>
                <p className="text-tan text-xl" style={{ fontFamily: '"Playfair Display", serif' }}>{v}</p>
                <p className="text-paper/40 text-[10px] tracking-widest uppercase mt-0.5">{l}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="w-full lg:w-[45%] flex items-center justify-center px-6 py-20 bg-paper">
        <div className="w-full max-w-[380px] animate-fade-up">
          ...
          <div className="lg:hidden mb-10 text-center">
            <Link to="/">
              <span className="text-espresso text-2xl" style={{ fontFamily: '"Playfair Display", serif' }}>
                Global Leather Hub
              </span>
            </Link>
          </div>
          ...
          <p className="text-fog text-sm font-light mb-8">Access your wholesale account and catalog.</p>

          <form onSubmit={handleSubmit} className="space-y-5">

            {/* Email */}
            <div className="animate-fade-up stagger-1">
              <label className="block text-[11px] tracking-widest uppercase text-fog font-medium mb-2">Email Address</label>
            ...
            <div className="animate-fade-up stagger-2">
              <div className="flex items-center justify-between mb-2">
                <label className="text-[11px] tracking-widest uppercase text-fog font-medium">Password</label>
                <Link to="/forgot-password" className="text-[11px] text-tan hover:text-sienna transition-colors link-underline">
                  Forgot password?
                </Link>
              </div>
              <div className="relative">
            ...
            <div className="animate-fade-up stagger-3">
              <CloudflareTurnstile
                onVerify={(t) => { setCfToken(t); setErrors((p) => ({ ...p, cf: "" })); }}
                onExpire={() => setCfToken("")}
                onError={() => setErrors((p) => ({ ...p, cf: "Verification failed. Please retry." }))}
              />
              {errors.cf && (
                <p className="mt-1.5 text-[11px] text-rust flex items-center gap-1 animate-fade-in">
                  <AlertCircle className="w-3 h-3" />{errors.cf}
                </p>
              )}
            </div>
            ...
            <p className="text-center text-fog text-[13px] animate-fade-up stagger-5">
              No account?{" "}
              <Link to="/register" className="text-tan hover:text-sienna font-medium transition-colors link-underline">
                Create one
              </Link>
            </p>
          </form>
