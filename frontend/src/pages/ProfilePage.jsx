...
const Err = ({ msg }) =>
  msg ? (
    <p className="mt-1.5 text-[11px] text-rust flex items-center gap-1">
...
const Section = ({ title, icon: Icon, action, children }) => (
  <div className="card hover:shadow-card transition-shadow duration-300">
    <div className="flex items-center justify-between mb-5">
      <h3
        className="flex items-center gap-2.5 text-espresso text-xl"
...
export default function ProfilePage() {
  ...
  const handleUsernameUpdate = async () => {
    ...
    if (!/^[a-zA-Z0-9_]+$/.test(newUsername)) {
      setUsernameErr("Letters, numbers, underscores only");
      return;
    }
  ...
  const handleOtpKeyDown = (i, e) => {
    if (e.key === "Backspace" && !pwOtp[i] && i > 0)
  ...
  const handleChangePassword = async () => {
    ...
    if (!newPw) e.password = "New password required";
      ...
      const f = requirements.filter((r) => !r.test(newPw));
      if (f.length) e.password = f[0].label + " required";
    }
          ...
          <div className="lg:col-span-1 space-y-4">
            <div className="card text-center hover:shadow-card transition-shadow">
              <div className="relative w-20 h-20 mx-auto mb-4">
                <div className="w-20 h-20 rounded-full overflow-hidden bg-linen border-2 border-border flex items-center justify-center">
                  {user?.avatar ? (
                    ...
                    <span
                      className="text-3xl text-sienna"
              ...
              <div className="mt-4 flex flex-wrap gap-2 justify-center">
                ...
                <span className="badge-tan">
                  {user?.role === "admin" ? "Admin" : "Buyer"}
                </span>
              </div>
              {user?.lastLogin && (
                <p className="text-fog/50 text-[10px] mt-4">
          ...
          <div className="lg:col-span-2 space-y-5">
                ...
                <div className="space-y-3">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`field pl-11 ${usernameErr ? "field-error" : ""}`}
            ...
            <Section title="Email Address" icon={Mail}>
              <div className="flex items-center gap-3 bg-linen border border-border rounded-xl px-4 py-3">
                ...
                <span className="text-espresso/80 text-sm flex-1">
                  {user?.email}
                </span>
                <CheckCircle className="w-4 h-4 text-sage flex-shrink-0" />
              </div>
              <p className="text-fog/50 text-[11px] mt-2">
            ...
            <Section title="Preferred Currency" icon={DollarSign}>
              <div className="space-y-3">
                <p className="text-fog text-sm">
                  Select your preferred currency for pricing display and quotes.
                </p>
                <select
              ...
              {pwStep === 0 && (
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    disabled={pwLoading}
                    className="btn-outline w-full justify-center text-[13px] py-3"
                  >
                    {pwLoading ? (
              ...
              {pwStep === 1 && (
                <div className="space-y-5">
                  ...
                  <p className="text-center text-fog text-[13px]">
                    Didn't get it?{" "}
                    {resendCD > 0 ? (
                      <span className="text-fog/40">Resend in {resendCD}s</span>
                    ) : (
                      <button
                        onClick={handleResend}
                        className="text-tan hover:text-sienna transition-colors"
                      >
                        Resend
                      </button>
                    )}
                  ...
                  <button
                    onClick={() => setPwStep(0)}
                    className="w-full text-[13px] text-fog/50 hover:text-fog transition-colors"
                  >
                    Cancel
                  </button>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`field pl-11 pr-11 ${pwErrors.password ? "field-error" : ""}`}
                    ...
                    {newPw && (
                      <div className="mt-3 grid grid-cols-2 gap-1.5">
                        {requirements.map((r) => (
                          <div
                            key={r.label}
                            className={`flex items-center gap-1.5 text-[11px] ${r.test(newPw) ? "text-sage" : "text-fog/40"}`}
                          >
                            <CheckCircle
                              className={`w-2.5 h-2.5 ${r.test(newPw) ? "text-sage" : "text-border"}`}
                            />
                            {r.label}
                          </div>
                        ))}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`field pl-11 pr-11 ${pwErrors.confirm ? "field-error" : ""}`}
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className="flex-1 btn-outline text-[13px] py-3"
                    >
                      Cancel
                    </button>
                    <button
