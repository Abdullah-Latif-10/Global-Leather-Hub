...
// Helper to create and configure nodemailer transporter
const getTransporter = () => {
  ...
  return nodemailer.createTransport({
    ...
    secure: parseInt(port, 10) === 465, // true for port 465, false for others
    auth: {
      user,
      pass
    }
...
const sendNodemailerEmail = async (to_email, subject, html, reply_to = null) => {
...
const sendOTPEmail = async (to, otp, purpose = 'verification') => {
  ...
  try {
    const info = await sendNodemailerEmail(to, subject, html);
...
const sendContactEmail = async (formData) => {
  ...
  try {
    const info = await sendNodemailerEmail('crisitiano678@gmail.com', `Direct Inquiry: ${inquiryType} - ${name}`, html, email);
...
const sendOrderStatusEmail = async (userOrUserId, subjectShort, order) => {
  ...
  try {
    const info = await sendNodemailerEmail(toEmail, subject, html);
